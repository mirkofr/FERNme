...
\large FERNme --- a user-owned Hebbian preference-graph memory with a zero-LLM deterministic core}
...
budget and is the only zero-model-call deterministic-core method that resolves preference drift. We argue that for
...
rule (zero-model-call deterministic core), and deployment (multi-tenant, user-owned, with a private
...
a write-time error surface. FERNme trades that richness for a zero-model-call
deterministic core and makes the trade-off explicit. Optional enrichment is
propose-only and requires human approval before memory truth changes.
...
\paragraph{Write rule (zero-model-call hot path).} An event maps to active attributes via a
deterministic function over structured fields and a catalog/controlled-vocabulary table
with no model call in the hot path. Optional enrichment is a separate propose-only
review-queue tier, not a truth-writing hot-path call. For each active attribute,
\[
...
per write; precision@5 vs.\ ground truth against zero-model-call baselines (frequency, recency)
...
and association-building in \texttt{pure} mode (zero-model-call deterministic write path).
...
top-$k$ retrieved facts. All compared systems are \textbf{zero-model-call retrieval baselines and runnable}: FERNme
...
\caption{Answer-retrieval accuracy at top-10: context-seeded FERNme vs.\ zero-model-call
...
This is a head-to-head among zero-model-call retrieval memories on real free-form text, at equal
...
from 86 entries of free-form text while preserving the zero-model-call deterministic core.
